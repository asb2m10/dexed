...
/**
 * This is a reimplementation of juce::CachedValue that adds a callback when a value changes
 */
...
class CachedValueCallback   : private juce::ValueTree::Listener
{
public:
    //==============================================================================
    /** Default constructor.
    ...
    */
    CachedValueCallback (juce::ValueTree& tree, const juce::Identifier& propertyID,
    ...
    */
    ...
    CachedValueCallback& operator= (const Type& newValue);
    ...
    void referTo (juce::ValueTree& tree, const juce::Identifier& property, juce::UndoManager* um);
...
private:
    ...
    //==============================================================================
    JUCE_DECLARE_WEAK_REFERENCEABLE (CachedValueCallback)
    JUCE_DECLARE_NON_COPYABLE (CachedValueCallback)
};


//==============================================================================
template <typename Type>
inline CachedValueCallback<Type>::CachedValueCallback() = default;

template <typename Type>
inline CachedValueCallback<Type>::CachedValueCallback (juce::ValueTree& v, const juce::Identifier& i, juce::UndoManager* um)
...
template <typename Type>
inline CachedValueCallback<Type>::CachedValueCallback (juce::ValueTree& v, const juce::Identifier& i, juce::UndoManager* um, const Type& defaultToUse)
    : targetTree (v), targetProperty (i), undoManager (um),
...
template <typename Type>
inline juce::Value CachedValueCallback<Type>::getPropertyAsValue()
{
    return targetTree.getPropertyAsValue (targetProperty, undoManager);
}

template <typename Type>
inline bool CachedValueCallback<Type>::isUsingDefault() const
{
    return ! targetTree.hasProperty (targetProperty);
}

template <typename Type>
inline CachedValueCallback<Type>& CachedValueCallback<Type>::operator= (const Type& newValue)
{
...
template <typename Type>
inline void CachedValueCallback<Type>::setValue (const Type& newValue, juce::UndoManager* undoManagerToUse)
{
    if (! exactlyEqual (cachedValue, newValue) || isUsingDefault())
    {
        cachedValue = newValue;
        targetTree.setProperty (targetProperty, juce::VariantConverter<Type>::toVar (newValue), undoManagerToUse);
    }
}

template <typename Type>
inline void CachedValueCallback<Type>::resetToDefault()
{
    resetToDefault (undoManager);
}

template <typename Type>
inline void CachedValueCallback<Type>::resetToDefault (juce::UndoManager* undoManagerToUse)
{
...
template <typename Type>
inline void CachedValueCallback<Type>::referTo (juce::ValueTree& v, const juce::Identifier& i, juce::UndoManager* um)
{
    referToWithDefault (v, i, um, Type());
}

template <typename Type>
inline void CachedValueCallback<Type>::referTo (juce::ValueTree& v, const juce::Identifier& i, juce::UndoManager* um, const Type& defaultVal)
{
    referToWithDefault (v, i, um, defaultVal);
}

template <typename Type>
inline void CachedValueCallback<Type>::forceUpdateOfCachedValue()
{
...
template <typename Type>
inline void CachedValueCallback<Type>::referToWithDefault (juce::ValueTree& v, const juce::Identifier& i, juce::UndoManager* um, const Type& defaultVal)
{
...
template <typename Type>
inline Type CachedValueCallback<Type>::getTypedValue() const
{
    if (const juce::var* property = targetTree.getPropertyPointer (targetProperty))
        return juce::VariantConverter<Type>::fromVar (*property);

    return defaultValue;
}

template <typename Type>
inline void CachedValueCallback<Type>::valueTreePropertyChanged (juce::ValueTree& changedTree, const juce::Identifier& changedProperty)
{
